...
const MusicDetailModal: React.FC<MusicDetailModalProps> = ({ song, playerRanking, onClose, onAddToQueue }) => {
  ...
  return (
    <div 
      className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-2 xs:p-3 sm:p-4 z-50 transition-opacity duration-300"
      style={{ opacity: animateIn ? 1 : 0 }}
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div 
        className={`bg-gradient-to-br from-indigo-900/90 via-purple-900/90 to-pink-900/90 backdrop-blur-lg p-3 xs:p-4 sm:p-6 rounded-xl xs:rounded-2xl shadow-2xl w-full max-w-3xl border border-white/20 relative overflow-hidden transition-all duration-500 transform ${animateIn ? 'scale-100 translate-y-0' : 'scale-95 translate-y-10'}`}
      >
        ...
        <button 
          onClick={onClose} 
          className="absolute top-2 xs:top-3 sm:top-4 right-2 xs:right-3 sm:right-4 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 rounded-full p-1.5 xs:p-2 transition-all duration-300 z-10 group"
        >
          <svg className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
          <span className="absolute right-full mr-2 top-1/2 -translate-y-1/2 bg-black/70 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Fechar (Esc)</span>
        </button>
        ...
        <div className="relative z-10">
          ...
          <button
            ...
            disabled={isAdding}
            className={`w-full group relative overflow-hidden py-3 xs:py-3.5 sm:py-4 rounded-lg xs:rounded-xl font-bold text-base xs:text-lg shadow-lg transition-all duration-500 transform hover:scale-105 ${selectedCoach ? 'bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white' : 'bg-white/20 text-white/70'}`}
          >
            ...
            <span className="relative z-10 flex items-center justify-center">
              {isAdding ? (
                ...
                <>
                  ...
                  {selectedCoach ? 'Adicionar à Fila de Dança' : 'Selecione um Coach'}
                </>
              )}
            </span>
          </button>
